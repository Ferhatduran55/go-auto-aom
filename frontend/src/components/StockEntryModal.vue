    ...
    <div class="modal-container">
      ...
      <div class="modal-body">
        ...
        <div v-if="activeTab === 'single'" class="form-section">
          <div class="form-group">
            ...
            <div class="autocomplete-wrapper" ref="autocompleteRef">
              ...
              <div v-if="selectedProductDetail" class="selected-product">
              ...
              <div v-if="showDropdown && filteredProducts.length > 0 && !selectedProductDetail" class="autocomplete-dropdown">
                ...
                >
                  ...
                  <div class="item-meta">
                    ...
                    <span v-if="p.category" class="item-category">{{ p.category }}</span>
                  </div>
                </div>
              </div>
        ...
        <div v-else class="form-section">
          ...
          <div class="bulk-list">
            <div v-for="(entry, index) in bulkEntries" :key="index" class="bulk-row">
...
const formValid = computed(() => {
  if (activeTab.value === 'single') {
    ...
    return bulkEntries.value.some(e => e.productId && e.amount > 0)
  }
})
  ...
  try {
    if (activeTab.value === 'single') {
      ...
      if (result.error) {
        ...
        emit('success', 'Stok girişi başarıyla tamamlandı')
        emit('close')
      }
    } else {
      ...
      if (validEntries.length === 0) {
        error.value = 'En az bir geçerli giriş yapmalısınız'
        return
      }
      
      const result = await bulkStockIn(validEntries)
      
      if (result.error) {
        error.value = result.error
      } else {
        const message = `${result.successful} ürüne stok girişi yapıldı`
...
watch(() => props.bulkProducts, (newProducts) => {
  if (newProducts && newProducts.length > 0) {
    ...
    bulkEntries.value = newProducts.map(p => ({
      productId: p.id,
      amount: 1,
      note: ''
    }))
...
.bulk-row {
  display: grid;
  grid-template-columns: 2fr 80px 1fr 32px;
  gap: 0.5rem;
  align-items: center;
}
